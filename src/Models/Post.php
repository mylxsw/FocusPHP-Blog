...
class Post implements Model
{
     ...
     *
     * @return array ['data'=> ..., 'page'=>[total, offset, count, page_nums,
     *                   current]]
     */
    public function getRecentlyPosts($current, $count = 10)
    {
        ...
        $sql
              = "SELECT * FROM `ar_article`  ORDER BY `publish_date` DESC LIMIT {$offset}, {$count}";
    ...
    {
        $where
                = 'id IN (SELECT re.article_id FROM ar_article_category re WHERE re.category_id = :cat)';
        $params = [':cat' => $cat];
        ...
        $sql
              = "SELECT * FROM `ar_article` WHERE {$where} ORDER BY `publish_date` DESC LIMIT {$offset}, {$count}";
        $stat = $this->getPDO()->prepare($sql);
        $stat->execute($params);
    ...
    {
        $where
                = 'id IN (SELECT re.article_id FROM ar_article_tag re WHERE re.tag_id=:tag)';
        $params = [':tag' => $tagId];
        $total  = $this->getPostCount($where, $params);
    ...
    {
        $stat = $this->getPDO()
            ->prepare('SELECT count(*) as count FROM `ar_article` '
    ...
    {
        $stat = $this->getPDO()
            ->prepare("SELECT * FROM `ar_article` WHERE id = :id");
     ...
     * @return array
     */
    public function getPostsInfoByTags(array $tags, $limit = 10)
    {
        if (empty($tags)) {
            return [];
        }
        ...
        $sql          = "SELECT art.id,
                ...
                WHERE art.isvalid=1
                AND art.id IN (
                  SELECT tag.article_id FROM `ar_article_tag` tag
                  WHERE tag.tag_id IN ({$placeHolders})) ORDER BY RAND() LIMIT {$limit}";
